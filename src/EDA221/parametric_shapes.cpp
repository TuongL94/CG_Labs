	...
	unsigned int const res_phi, float const radius)
{
	...
	float theta = 0.0f;
	float dtheta = 2.0f * bonobo::pi / (static_cast<float>(res_theta - 1)); // Step size for theta

	float phi = 0.0f;
	float dphi = bonobo::pi / (static_cast<float>(res_phi - 1)); // Step size for phi
